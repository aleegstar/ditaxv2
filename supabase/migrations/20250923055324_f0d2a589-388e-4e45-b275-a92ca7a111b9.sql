...
-- Add security audit logging for rate limit access
CREATE OR REPLACE FUNCTION log_rate_limit_access()
RETURNS TRIGGER AS $$
BEGIN
  -- Log rate limit access attempts
...
-- Create trigger for rate limit access logging
DROP TRIGGER IF EXISTS rate_limit_access_trigger ON rate_limits;
CREATE TRIGGER rate_limit_access_trigger
  AFTER SELECT ON rate_limits
  FOR EACH STATEMENT
  EXECUTE FUNCTION log_rate_limit_access();
