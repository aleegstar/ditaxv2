...
const UserTaxReturns = () => {
  ...
  const [profile, setProfile] = useState<{ onboarding_tour_completed?: boolean } | null>(null);
  ...
  return (
    <div className="min-h-screen bg-[#020408] text-zinc-100 antialiased selection:bg-[#1D64FF]/30">
      {/* Mobile Container */}
      <div className="overflow-hidden min-h-screen md:max-w-2xl w-full max-w-[430px] mx-auto relative">
        ...
        <div className="px-8 mb-10 flex justify-between items-center relative z-20">
          <div className="flex items-center gap-4">
            <div className="relative shrink-0">
              <div className="w-14 h-14 rounded-full p-[2px] bg-gradient-to-br from-[#1D64FF] to-[#1D64FF]/10">
                <img 
                  src={userProfile?.avatar_url || '/lovable-uploads/default-avatar.png'} 
                  alt="Profile" 
          ...
          <button 
            onClick={() => createNewTaxReturn(`${currentYear - 1}`)}
            className="w-12 h-12 rounded-full text-white flex items-center justify-center transition-colors bg-gradient-to-br from-[#1D64FF] to-[#3B82F6] hover:from-[#1D64FF] hover:to-[#2563EB] shadow-[0_12px_30px_-8px_rgba(29,100,255,0.6)]"
          >
        ...
        <div className="relative px-4 flex flex-col gap-5 md:px-8 z-20 pb-24">
                ...
                <div className="space-y-1">
                  <div className="flex items-center justify-between text-sm">
                    ...
                    <span className="text-zinc-400 font-medium font-jakarta group-hover:text-zinc-300 transition-colors">
                      {existingReturn?.updated_at 
                        ? new Date(existingReturn.updated_at).toLocaleDateString('de-CH', { day: 'numeric', month: 'short', year: 'numeric' })
                        : '–'
                      }
                    </span>
                  </div>
          ...
          {inProgressYears.length === 0 && (
            <motion.div
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ delay: 0.2, duration: 0.4, ease: 'easeOut' }}
              onClick={() => createNewTaxReturn(`${currentYear - 1}`)}
              className="group relative w-full border border-dashed border-white/10 rounded-[1.5rem] p-6 hover:border-[#1D64FF]/50 transition-all duration-300 cursor-pointer bg-gradient-to-b from-[#0a0a0a] to-[#020202]"
            >
