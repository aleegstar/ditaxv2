    ...
    <>
      ...
      <div className="rounded-3xl border border-border bg-card divide-y divide-border overflow-hidden">
        {files.map((f) => {
          ...
          const profile = f.suggestedChecklistItemId ? getDocumentProfile(f.suggestedChecklistItemId) : null;
          return (
            <div key={f.id} className="p-4 md:p-5 flex flex-col md:flex-row md:items-center gap-3 md:gap-4">
              ...
              <div className="flex items-center gap-2 flex-1 min-w-0">
                <div className="relative flex-1 min-w-0">
                  <select
                    ...
                    className={cn(
                      ...
                      'border-border focus:outline-none focus:ring-2 focus:ring-[#1E3A5F]/20',
                      !f.suggestedChecklistItemId && 'text-muted-foreground',
                    )}
                  >
                ...
                >
                  {f.confidence > 0 ? `${Math.round(f.confidence)}%` : '–'}
                </span>
                <button
                  type="button"
      ...
      <div className="sticky bottom-4 mt-6 z-10">
        <div className="rounded-2xl border border-border bg-card/95 backdrop-blur p-4 flex items-center justify-between gap-4 shadow-sm">
          <div className="text-sm">
            <span className="font-semibold text-foreground">{files.filter(f => f.suggestedChecklistItemId).length}</span>
            <span className="text-muted-foreground"> von {files.length} bereit zum Hochladen</span>
          </div>
          <Button onClick={handleConfirmUpload} disabled={!allAssigned}>
