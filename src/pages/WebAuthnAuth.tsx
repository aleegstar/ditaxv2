...
const WebAuthnAuth = () => {
  ...
  const [step, setStep] = useState<"email" | "authenticating" | "error">("email");
  ...
  return <div className="min-h-screen flex items-center justify-center bg-background px-[30px]">
      <div className="w-full max-w-md space-y-4">
        ...
        {step === "email" && <div className="space-y-6">
            ...
            <form onSubmit={handleEmailSubmit} className="space-y-6">
              <div>
                <Input type="email" placeholder="E-Mail-Adresse" value={email} onChange={e => setEmail(e.target.value)} className="h-14 px-6 py-4 text-base rounded-full border border-[#E2E8F0] bg-white text-foreground placeholder:text-muted-foreground shadow-none focus:shadow-none focus-visible:ring-0 focus-visible:ring-offset-0 focus-visible:border-[#E2E8F0]" required disabled={isLoading} />
              </div>
              <Button type="submit" disabled={isLoading || !email} className="w-full bg-[#1d64ff] hover:bg-[#1d64ff]/90 text-white rounded-full px-[20px] py-[10px] h-14 text-base font-medium border-0 transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-[10px]" style={{
            ...
            <Button onClick={handleBackToMain} className="w-full bg-white hover:bg-gray-50 text-black border border-[rgb(230,230,230)] font-medium px-[20px] py-[10px] h-14 text-base rounded-full transition-colors duration-200 flex items-center justify-center gap-[10px]" style={{
        ...
        {step === "authenticating" && <div className="space-y-6">
            <div className="space-y-2">
            ...
            <Button onClick={handleRetry} className="w-full bg-white hover:bg-gray-50 text-black border border-[rgb(230,230,230)] font-medium px-[20px] py-[10px] h-14 text-base rounded-full transition-colors duration-200 flex items-center justify-center gap-[10px]" style={{
          boxShadow: 'rgba(0, 0, 0, 0.05) 0px 3px 5px 0px'
        }}>
              Abbrechen
            </Button>
        ...
        {step === "error" && <div className="space-y-6">
            <div className="space-y-2">
            ...
            <div className="space-y-3">
              {error.includes("Keine Fingerprint-Geräte") ? <div className="text-sm text-black/60 bg-muted/50 rounded-xl p-4">
                  <p className="font-medium mb-2">Mögliche Lösungen:</p>
                  <ul className="list-disc list-inside space-y-1 text-left">
                    <li>Registrieren Sie zuerst ein Fingerprint-Gerät</li>
                    <li>Verwenden Sie die E-Mail-Anmeldung</li>
                    <li>Prüfen Sie die eingegebene E-Mail-Adresse</li>
                  </ul>
                </div> : <Button onClick={handleRetry} className="w-full bg-[#1d64ff] hover:bg-[#1d64ff]/90 text-white rounded-full px-[20px] py-[10px] h-14 text-base font-medium border-0 transition-colors duration-200" style={{
            boxShadow: 'rgba(29, 100, 255, 0.2) 0px 3px 10px 0px'
          }}>
                  Erneut versuchen
                </Button>}
