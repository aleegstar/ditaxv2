...
import { Plus, ChevronRight, Users, UserCog, Pencil, Trash2, Loader2 } from 'lucide-react';
          ...
          <div className="relative h-32 md:h-48 w-full overflow-hidden bg-muted">
            ...
            <div className="absolute top-3 left-3 inline-flex items-center gap-1.5 px-2 py-1 rounded-full bg-card/90 backdrop-blur-sm border border-border/60">
              ...
              <span className="text-[11px] font-medium text-foreground">
                {editMode ? 'Profile bearbeiten' : 'Profil wählen'}
              </span>
            </div>
          </div>
        ...
        <div className="flex items-center justify-between px-1 mb-2.5">
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className="text-[11px] font-semibold uppercase tracking-[0.12em] text-primary/80 hover:text-primary transition-colors"
            >
          ...
          <>
            <div className="bg-white border border-black/[0.07] rounded-2xl overflow-hidden shadow-[0_1px_2px_rgba(15,27,61,0.03),0_4px_16px_-12px_rgba(15,27,61,0.06)] divide-y divide-black/[0.05]">
              {taxFilers.map((filer) => {
                ...
                const Row: any = editMode ? 'div' : 'button';
                return (
                  <Row
                    key={filer.id}
                    {...(!editMode && {
                    ...
                    className={`w-full group relative text-left flex items-center gap-4 md:gap-5 px-5 py-4 md:py-5 transition-colors duration-150 ${
                      editMode
                        ? ''
                        : 'cursor-pointer hover:bg-foreground/[0.022] active:bg-foreground/[0.04] focus:outline-none focus-visible:bg-foreground/[0.03]'
                    }`}
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[12px] md:text-[13px] text-muted-foreground/65 truncate mt-0.5">
                        {relationshipLabel}
                        {editMode && filer.date_of_birth && ` • ${new Date(filer.date_of_birth).toLocaleDateString('de-CH')}`}
                      </p>
                    </div>
