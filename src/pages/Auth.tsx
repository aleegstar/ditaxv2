                ...
                <form onSubmit={handleEmailSubmit} className="space-y-5">
                  <div className="space-y-1.5">
                    ...
                    <div className="relative">
                      <input type="email" name="email" id="email" value={email} onChange={e => setEmail(e.target.value)} onFocus={() => setIsInputFocused(true)} onBlur={() => setTimeout(() => setIsInputFocused(false), 150)} className="block w-full min-h-[56px] px-6 py-4 text-base rounded-xl border border-slate-200 bg-white text-slate-800 placeholder:text-slate-400 shadow-none focus:shadow-none focus:outline-none focus:border-[#1D64FF] transition-all font-jakarta" placeholder="name@firma.com" required disabled={isLoading} />
                    </div>
                  </div>
