                ...
                <form onSubmit={handleEmailSubmit} className="space-y-5">
                  <div className="space-y-1.5">
                    ...
                    <div className="relative">
                      <input type="email" name="email" id="email" value={email} onChange={e => setEmail(e.target.value)} onFocus={() => setIsInputFocused(true)} onBlur={() => setTimeout(() => setIsInputFocused(false), 150)} className="flex w-full shadow-sm shadow-black/5 transition-shadow bg-slate-50 text-slate-800 text-base py-4 px-6 rounded-xl focus:border-[#1D64FF] focus:ring-[#1D64FF]/20 focus:outline-none placeholder:text-slate-400 font-jakarta disabled:cursor-not-allowed disabled:opacity-50" placeholder="name@firma.com" required disabled={isLoading} />
                    </div>
                  </div>
