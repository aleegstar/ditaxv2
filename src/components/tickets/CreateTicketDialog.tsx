...
export const CreateTicketDialog = ({
  ...
  return <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogContent hideCloseButton className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-[calc(100%-2rem)] max-w-md max-h-[90vh] bg-white border-0 p-6 overflow-hidden shadow-[0_25px_50px_-12px_rgba(0,0,0,0.25)] rounded-3xl gap-0">
        {/* Close Button */}
        <button
          onClick={handleClose}
          ...
          <div className="pt-8">
            ...
            <div className="flex flex-col items-center mb-6">
              <div className="w-16 h-16 rounded-2xl bg-orange-50 flex items-center justify-center mb-4">
                <MessageSquarePlus className="w-8 h-8 text-orange-500" strokeWidth={1.5} />
              </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <Textarea 
                  ...
                  className="bg-white border-slate-200 min-h-[100px] rounded-xl text-slate-900 placeholder:text-slate-400 focus:border-blue-500 focus:ring-blue-500/20 resize-none" 
                  required 
                />
              </div>
              ...
              <div className="space-y-2">
                ...
                <label className="flex items-center gap-3 p-3 rounded-xl border border-slate-200 bg-white hover:bg-slate-50 transition-colors cursor-pointer">
                  <span className="text-sm text-slate-600">Durchsuchen...</span>
                  <span className="text-sm text-slate-400 italic">
                    {attachments.length === 0 ? 'Keine Datei ausgewählt.' : `${attachments.length} Datei(en) ausgewählt`}
                  </span>
                  <Input 
                    type="file" 
                    multiple 
                    ...
                    {attachments.map((file, index) => (
                      <div key={index} className="flex items-center justify-between bg-slate-50 p-3 rounded-xl border border-slate-100">
                        ...
                        <Button 
                          ...
                          className="text-slate-400 hover:text-slate-600 hover:bg-slate-200 rounded-lg h-8 w-8 p-0"
                        >
                          <X className="h-4 w-4" />
                        </Button>
                      </div>
