...
export const ChatBotInterface: React.FC<ChatBotInterfaceProps> = ({
    ...
    const checkBotHandover = async () => {
      ...
      try {
        const { data, error } = await supabase
          .from('chat_messages')
          .select('bot_handover_requested, id')
        ...
        if (data && data.length > 0) {
          ...
          await supabase
            .from('chat_messages')
            .update({ bot_handover_requested: false })
            .eq('id', data[0].id);
          toast({
  ...
  const loadChatHistory = async () => {
    ...
    try {
      ...
      if (data && data.length > 0) {
        const recentHandover = data.find(msg => 
          msg.bot_handover_requested && 
          new Date(msg.created_at) > new Date(Date.now() - 5 * 60 * 1000)
        );
        if (recentHandover) {
          setEscalatedMode(false);
        }
        ...
        const adminIds = [...new Set(
          data
            .filter(msg => msg.sender_id && msg.sender_id !== userId && msg.chat_type === 'human')
    ...
    try {
      if (escalatedMode) {
        ...
        const { data, error } = await supabase.functions.invoke('chatbot-response', {
          body: { message: messageContent, userId, sessionId }
        });
  ...
  return (
    <>
      <div className="flex flex-col h-full bg-[#020408] relative overflow-hidden">
        ...
        <div 
          className="absolute top-0 left-0 w-full h-[400px] z-0 pointer-events-none opacity-100"
          style={{
        ...
        <div className="z-20 w-full px-4 sm:px-6 pt-4 sm:pt-6 pb-4 flex items-center gap-3 sm:gap-4 border-b border-white/[0.04] bg-[#020408]/80 backdrop-blur-xl shrink-0">
          <button 
            ...
            className="w-10 h-10 rounded-full border border-white/[0.08] bg-white/[0.02] flex items-center justify-center text-zinc-400 hover:text-white hover:border-white/20 hover:bg-white/[0.06] transition-all duration-300 shadow-lg shadow-black/40"
          >
          ...
          <div className="flex items-center gap-3">
            {/* Bot Avatar */}
            <div className="w-10 h-10 rounded-full flex items-center justify-center shadow-[0_0_15px_-3px_rgba(29,100,255,0.3)] border border-[#1D64FF]/30 relative overflow-hidden"
              style={{ background: escalatedMode ? 'linear-gradient(to bottom right, #059669, #047857)' : 'linear-gradient(to bottom right, #1D64FF, #0B2566)' }}
            >
              {escalatedMode ? (
          ...
          {isLoadingHistory ? (
            ...
            <div className="max-w-2xl mx-auto space-y-4">
              ...
              {messages.map(message => (
                <div 
                  key={message.id} 
                  className={`flex ${message.isBot || message.isAdmin ? 'items-start gap-3' : 'flex-col items-end'} ${message.isBot || message.isAdmin ? 'max-w-[90%]' : 'ml-auto max-w-[85%]'}`}
                >
                  {/* Bot/Admin Avatar */}
                  {(message.isBot || message.isAdmin) && (
                    <div 
                      className="w-8 h-8 rounded-full flex-shrink-0 flex items-center justify-center shadow-lg border mt-1 overflow-hidden"
                      style={{ 
                  ...
                  <div className="flex flex-col gap-1">
                    {message.isAdmin && message.senderName && (
                      <p className="text-xs font-medium text-zinc-500 ml-1">{message.senderName}</p>
                    )}
                    <div 
                      className={`px-4 py-3.5 rounded-[24px] text-sm leading-relaxed ${
                        message.isBot || message.isAdmin
                          ? 'bg-gradient-to-br from-[#181B21] to-[#0E1014] border border-white/[0.06] text-zinc-300 shadow-[0_2px_10px_-4px_rgba(0,0,0,0.5),inset_0_1px_0_rgba(255,255,255,0.05)]'
                          : 'bg-gradient-to-br from-[#2F75FF] to-[#0055FF] border border-white/10 text-white shadow-[0_8px_20px_-6px_rgba(29,100,255,0.3),inset_0_1px_0_rgba(255,255,255,0.2)]'
                      }`}
                    >
                ...
                <div className="flex items-start gap-3 max-w-[90%]">
                  ...
                  <div className="bg-gradient-to-br from-[#181B21] to-[#0E1014] border border-white/[0.06] px-4 py-3.5 rounded-[24px] shadow-[0_2px_10px_-4px_rgba(0,0,0,0.5),inset_0_1px_0_rgba(255,255,255,0.05)]">
                    <div className="flex gap-1">
                      ...
                      <div className="w-2 h-2 rounded-full bg-zinc-500 animate-bounce" style={{ animationDelay: '0.1s' }} />
        ...
        <div className="z-20 p-4 w-full bg-gradient-to-t from-[#020408] via-[#020408] to-transparent shrink-0 pb-24 md:pb-8">
          <div className="max-w-2xl mx-auto">
            <PromptInputBox 
              ...
              className="!bg-[#0A0C10] !border-white/[0.08] hover:!border-white/[0.12] focus-within:!border-[#1D64FF]/50 focus-within:!ring-1 focus-within:!ring-[#1D64FF]/20 shadow-2xl"
            />
      ...
      <ChatDeleteConfirmDialog 
        ...
        onConfirm={clearMessagesCompletely} 
