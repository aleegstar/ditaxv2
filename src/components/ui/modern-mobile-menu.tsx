...
const CustomFolderIcon: React.FC<{
  ...
  style
    ...
    <path 
      fillRule="evenodd" 
      clipRule="evenodd" 
      d="M44 34V22C44 17.5817 40.4183 14 36 14H29.7082C28.1931 14 26.8081 13.144 26.1305 11.7889L24.8944 9.31672C23.8781 7.28401 21.8005 6 19.5279 6H10C6.68629 6 4 8.68629 4 12V34C4 38.4183 7.58172 42 12 42H36C40.4183 42 44 38.4183 44 34ZM22.5528 13.5777C22.6246 13.7213 22.7004 13.8621 22.7799 14H14C12.8954 14 12 14.8954 12 16C12 17.1046 12.8954 18 14 18H36C38.2091 18 40 19.7909 40 22V34C40 36.2091 38.2091 38 36 38H12C9.79086 38 8 36.2091 8 34V12C8 10.8954 8.89543 10 10 10H19.5279C20.2854 10 20.9779 10.428 21.3167 11.1056L22.5528 13.5777Z" 
      fill="currentColor"
    />
  </svg>;
...
const CustomSendIcon: React.FC<{ className?: string }> = ({ className }) => (
  <svg viewBox="0 0 48 48" fill="currentColor" className={className}>
...
const CustomSettingsIcon: React.FC<{ className?: string }> = ({ className }) => (
...
const MobileMenuSheet: React.FC<MobileMenuSheetProps> = ({ isOpen, onOpenChange, navigate, location }) => {
  ...
  const navigationItems = [
    { label: 'Steuern', icon: CustomHomeIcon, route: '/' },
    { label: 'Dokumente', icon: CustomFolderIcon, route: '/documents' },
    { label: 'Chat', icon: CustomSendIcon, route: '/chat' },
  ];
  ...
  const helpItems = [
    ...
    { label: 'Hilfe', icon: HelpCircle, route: '/help' },
    { label: 'Feedback', icon: MessageSquare, route: '/feedback' },
    { label: 'Roadmap', icon: MapPin, route: '/roadmap' },
  ];

  const legalItems = [
    { label: 'Datenschutz', icon: Shield, route: '/privacy' },
    { label: 'Nutzungsbedingungen', icon: FileCheck, route: '/terms' },
    { label: 'Cookie-Richtlinie', icon: Cookie, route: '/cookies' },
    { label: 'Cookie-Einstellungen', icon: Cookie, action: () => { onOpenChange(false); } },
    { label: 'Datenschutz-Einstellungen', icon: Settings, route: '/privacy-settings' },
  ];

  const handleNavigation = (item: { route?: string; action?: () => void }) => {
    if (item.action) {
      item.action();
    } else if (item.route) {
      navigate(item.route);
  ...
  const MenuItem = ({ item, isActive }: { item: { label: string; icon: React.ElementType; route?: string; action?: () => void }; isActive?: boolean }) => {
    ...
    return (
      <button
        ...
        className={`w-full flex items-center gap-4 px-4 py-3 transition-colors text-left ${
          isActive 
            ? 'bg-[#1D64FF]/10 text-slate-900 border-l-2 border-[#1D64FF]' 
        ...
        <div className="border-t border-slate-200">
          ...
          >
            {profile?.avatar_url ? (
              ...
              <div className="w-10 h-10 rounded-full bg-slate-100 flex items-center justify-center text-slate-600 font-medium border border-slate-200">
...
const InteractiveMenu: React.FC<InteractiveMenuProps> = ({
  ...
  const {
    ...
    hideOnScroll: false,
    hideOnKeyboard: false, // We position above keyboard instead
    showOnScrollUp: true,
    debounceMs: 150
  });
    ...
    <AnimatePresence>
      {isInitialLoaded && shouldShow && (
        ...
        >
          ...
          >
            <div 
              className="inline-flex items-center gap-1 rounded-full px-1.5 py-1.5 backdrop-blur-xl bg-white/80 ring-1 ring-black/5"
              style={{
            ...
            >
              ...
              <motion.button
                ...
                className={`flex items-center gap-2 rounded-full transition-all ${
                  location.pathname === '/' 
                    ? 'bg-slate-100/80 px-4 py-2.5' 
                    : 'w-10 h-10 justify-center hover:bg-slate-100/50'
                }`}
              >
                <CustomHomeIcon 
                  className="w-5 h-5" 
                ...
                />
                {location.pathname === '/' && (
                  <span className="text-slate-900 text-sm font-medium">Steuern</span>
                )}
              </motion.button>
              ...
              >
                ...
                />
                {location.pathname === '/documents' && (
                  <span className="text-slate-900 text-sm font-medium">Dokumente</span>
                )}
              </motion.button>
