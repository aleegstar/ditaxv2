...
import { ArrowUp, Paperclip, Square, X, User } from "lucide-react";
...
export const PromptInputBox = React.forwardRef<HTMLDivElement, PromptInputBoxProps>((props, ref) => {
  ...
  const handleSubmit = () => {
    if (input.trim() || files.length > 0) {
      let messagePrefix = "";
      if (showSearch) messagePrefix = "[Mit Mitarbeitern sprechen: ";
        ...
        <PromptInputActions className="flex items-center justify-between gap-2 p-0 pt-2">
          <div className="flex items-center gap-1 transition-opacity duration-300 opacity-100 visible">
            ...
            >
              <div className="w-5 h-5 flex items-center justify-center flex-shrink-0">
                ...
                >
                  <User className={cn("w-4 h-4", showSearch ? "text-blue-600" : "text-inherit")} />
                </motion.div>
              </div>
              <AnimatePresence>
                {showSearch && (
          ...
          >
            <Button
              variant="ghost"
              size="icon"
              className="h-8 w-8 rounded-full bg-gradient-to-b from-[hsl(222,100%,60%)] to-[hsl(222,100%,47%)] hover:scale-[1.02] !text-white transition-all duration-200 disabled:opacity-70 disabled:cursor-not-allowed"
              onClick={() => {
