...
export function Book(props: BookProps) {
  const {
    ...
    texture,
    variant = 'default',
    textColor = '#221F26', // Dark Charcoal as default text color
    illustration,
    width,
    onClick,
    className,
  ...
  return (
    <div
      onClick={onClick}
      className={cn('w-fit [perspective:900px] inline-block group cursor-pointer', className)}
      style={
        {
          '--book-color': color,
    ...
    >
      <div 
        className="contain-inline-size aspect-[49/60] w-fit rotate-0 relative [transform-style:preserve-3d] min-w-[calc(var(--book-width))] transition-transform duration-500 ease-out group-hover:[transform:rotateY(-20deg)_scale(1.066)translateX(-8px)]"
        style={{ color: textColor }}
      >
        <Stack
          align="stretch"
          className={cn(
            "rounded-l border border-border rounded-r shadow-book bg-stone-100 dark:bg-stone-800 bg-[var(--book-color)] size-full absolute overflow-hidden",
          )}
        >
          {variant !== 'simple' && (
            <Stack
          ...
          <Stack grow={variant === 'simple'} direction="row" className="h-fit">
            ...
            <div 
              className="contain-inline-size w-full"
              style={{ color: textColor }}
            >
              {children}
            </div>
