...
export const TaxYearDashboard: React.FC = () => {
  ...
  return <div className="text-slate-900 antialiased min-h-screen bg-white">
      ...
      <div data-tour="form-step-1" onClick={() => setIsAngabenExpanded(true)} className="bg-gradient-to-b from-white to-slate-50/80 p-6 rounded-[1.5rem] ring-1 ring-slate-200/60 flex items-center gap-4 transition-all duration-300 cursor-pointer shadow-[0_4px_14px_0_rgba(100,116,139,0.12),0_20px_40px_-12px_rgba(0,0,0,0.06)] hover:shadow-[0_6px_20px_rgba(100,116,139,0.18)] hover:-translate-y-0.5">
          ...
          </div>) : (/* Expanded Card View */
      <section data-tour="form-step-1" className={`bg-gradient-to-b from-white to-slate-50/80 rounded-[2.5rem] ring-1 overflow-hidden relative transition-all duration-300 ${!allAngabenComplete ? 'shadow-[0_6px_20px_rgba(100,116,139,0.18),0_25px_50px_-12px_rgba(0,0,0,0.1)] ring-slate-300' : 'shadow-[0_4px_14px_0_rgba(100,116,139,0.12),0_20px_40px_-12px_rgba(0,0,0,0.06)] ring-slate-200/60'}`}>
            {/* BorderBeam for active step */}
            {!allAngabenComplete && <BorderBeam size={150} duration={10} borderWidth={1} colorFrom="#3B82F6" colorTo="#60A5FA" delay={0} />}
            {/* Step Header - clickable to collapse when complete */}
            <div onClick={() => allAngabenComplete && setIsAngabenExpanded(false)} className={`p-6 flex items-center gap-4 ${allAngabenComplete ? 'cursor-pointer hover:bg-slate-50/50' : ''}`}>
              ...
              {!allAngabenComplete && <div className="w-32 hidden md:block">
              ...
              {allAngabenComplete && <ChevronDown className="w-5 h-5 text-slate-300 rotate-180" />}
            </div>
        ...
        <section className={`flex flex-col gap-4 ${!allAngabenComplete ? 'opacity-50 grayscale select-none cursor-not-allowed' : ''}`}>
          ...
          <div data-tour="form-step-2" onClick={() => allAngabenComplete && handleDocumentsClick()} className={`bg-gradient-to-b from-white to-slate-50/80 p-6 rounded-[2.5rem] ring-1 flex items-center gap-4 transition-all duration-300 relative overflow-hidden ${allAngabenComplete && !isDocumentsComplete ? 'shadow-[0_6px_20px_rgba(100,116,139,0.18),0_25px_50px_-12px_rgba(0,0,0,0.1)] ring-slate-300 cursor-pointer hover:shadow-[0_8px_25px_rgba(100,116,139,0.22),0_30px_60px_-12px_rgba(0,0,0,0.12)] hover:-translate-y-1' : allAngabenComplete ? 'shadow-[0_4px_14px_0_rgba(100,116,139,0.12),0_20px_40px_-12px_rgba(0,0,0,0.06)] ring-slate-200/60 cursor-pointer hover:shadow-[0_6px_20px_rgba(100,116,139,0.18)] hover:-translate-y-0.5' : 'shadow-[0_4px_14px_0_rgba(100,116,139,0.12),0_20px_40px_-12px_rgba(0,0,0,0.06)] ring-slate-200/60'}`}>
            {/* BorderBeam for active step */}
            {allAngabenComplete && !isDocumentsComplete && <BorderBeam size={150} duration={10} borderWidth={1} colorFrom="#3B82F6" colorTo="#60A5FA" delay={0} />}
          ...
          <div data-tour="form-step-3" onClick={() => canSubmit && handleSubmitClick()} className={`bg-gradient-to-b from-white to-slate-50/80 p-6 rounded-[2.5rem] ring-1 flex items-center gap-4 transition-all duration-300 relative overflow-hidden ${canSubmit && !isCompleted('submit') ? 'shadow-[0_6px_20px_rgba(100,116,139,0.18),0_25px_50px_-12px_rgba(0,0,0,0.1)] ring-slate-300 cursor-pointer hover:shadow-[0_8px_25px_rgba(100,116,139,0.22),0_30px_60px_-12px_rgba(0,0,0,0.12)] hover:-translate-y-1' : canSubmit ? 'shadow-[0_4px_14px_0_rgba(100,116,139,0.12),0_20px_40px_-12px_rgba(0,0,0,0.06)] ring-slate-200/60 cursor-pointer hover:shadow-[0_6px_20px_rgba(100,116,139,0.18)] hover:-translate-y-0.5' : 'shadow-[0_4px_14px_0_rgba(100,116,139,0.12),0_20px_40px_-12px_rgba(0,0,0,0.06)] ring-slate-200/60'} ${!allAngabenComplete ? '' : !isDocumentsComplete ? 'opacity-50 grayscale select-none cursor-not-allowed' : ''}`}>
            {/* BorderBeam for active step */}
            {canSubmit && !isCompleted('submit') && <BorderBeam size={150} duration={10} borderWidth={1} colorFrom="#3B82F6" colorTo="#60A5FA" delay={0} />}
