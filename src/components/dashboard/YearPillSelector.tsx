...
export const YearPillSelector: React.FC<YearPillSelectorProps> = ({
  ...
  return (
    <div className="inline-flex items-center gap-1 p-1 sm:p-1.5 rounded-full bg-foreground/[0.045] overflow-x-auto max-w-full [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
      {years.map(year => {
        ...
        return (
          <button
            ...
            className={cn(
              'shrink-0 px-4 sm:px-6 h-9 sm:h-11 rounded-full text-sm sm:text-base tabular-nums transition-all duration-200 active:scale-[0.97]',
              active
