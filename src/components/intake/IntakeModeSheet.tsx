...
const ModeCard: React.FC<{
  ...
  cta: string;
  onClick: () => void;
}> = ({ dark, active, image, imageAlt, icon, badge, title, desc, cta, onClick }) => (
  <button
    type="button"
    onClick={onClick}
    className={`group relative text-left rounded-2xl border overflow-hidden transition-all focus:outline-none focus:ring-2 focus:ring-primary/30 flex flex-col ${
      dark
        ? `bg-[#0F1B3D] hover:shadow-[0_12px_32px_rgba(15,27,61,0.35)] ${active ? "border-primary ring-2 ring-primary/20" : "border-[#1E3A5F]"}`
        : `bg-card hover:shadow-[0_8px_24px_rgba(15,27,61,0.08)] ${active ? "border-primary ring-2 ring-primary/20" : "border-border hover:border-primary/30"}`
    }`}
  >
    {dark && (
    ...
    <div className={`relative h-32 w-full overflow-hidden ${dark ? "bg-[#0F1B3D]" : "bg-muted"}`}>
      <img
        src={image}
        alt={imageAlt}
        loading="lazy"
    ...
    <div className="relative p-5 flex flex-col flex-1">
      <h3 className={`text-[15px] font-semibold tracking-tight ${dark ? "text-white" : "text-foreground"}`}>{title}</h3>
      <p className={`text-[13px] mt-1.5 leading-relaxed flex-1 ${dark ? "text-white/70" : "text-muted-foreground"}`}>{desc}</p>
      <div className={`mt-3 inline-flex items-center gap-1.5 text-[13px] font-medium ${dark ? "text-white" : "text-primary"}`}>
        {cta}
